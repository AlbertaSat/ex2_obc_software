...
static char *stringdup(const char *string) { return strcpy((char *)malloc(strlen(string) + 1), string); }
...
static char *end_of_token(char *token) { return token + strlen(token); }

static char last_char_of(char *token) { return *(end_of_token(token) - 1); }
