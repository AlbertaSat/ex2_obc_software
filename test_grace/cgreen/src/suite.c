...
bool has_setup(TestSuite *suite) { return (suite->setup != &do_nothing); }

bool has_teardown(TestSuite *suite) { return (suite->teardown != &do_nothing); }
...
void set_setup(TestSuite *suite, void (*set_up)(void)) { suite->setup = set_up; }

void set_teardown(TestSuite *suite, void (*tear_down)(void)) { suite->teardown = tear_down; }
