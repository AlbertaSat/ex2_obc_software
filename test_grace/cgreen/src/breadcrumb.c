...
void push_breadcrumb(CgreenBreadcrumb *breadcrumb, const char *name) {
    ...
    if (breadcrumb->depth > breadcrumb->space) {
        const char **tmp;
        breadcrumb->space++;
        tmp = (const char **)realloc((void *)breadcrumb->trail, sizeof(const char *) * breadcrumb->space);
        if (tmp == NULL) {
            breadcrumb->space--;
            breadcrumb->depth--;
            return;
...
void pop_breadcrumb(CgreenBreadcrumb *breadcrumb) { breadcrumb->depth--; }
...
int get_breadcrumb_depth(CgreenBreadcrumb *breadcrumb) { return breadcrumb->depth; }
