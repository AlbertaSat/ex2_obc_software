...
void assert_core_(const char *file, int line, const char *actual_string, intptr_t actual, Constraint *constraint) {
    ...
    if (NULL != constraint && is_not_comparing(constraint)) {
        (*get_test_reporter()->assert_true)(
            get_test_reporter(), file, line, false,
    ...
    if (parameters_are_not_valid_for(constraint, actual)) {
        ...
        (*get_test_reporter()->assert_true)(get_test_reporter(), file, line, false, validation_message);
...
void assert_that_double_(const char *file, int line, const char *expression, double actual,
    ...
    (*get_test_reporter()->assert_true)(
        get_test_reporter(), file, line, (*constraint->compare)(constraint, make_cgreen_double_value(actual)),
        "Expected [%s] to [%s] [%s] within [%d] significant figures\n"
        "\t\tactual value:\t\t\t[%08f]\n"
        "\t\texpected value:\t\t\t[%08f]",
        expression, constraint->name, constraint->expected_value_name, get_significant_figures(), actual,
...
void assert_equal_(const char *file, int line, const char *expression, intptr_t tried, intptr_t expected) {
    (*get_test_reporter()->assert_true)(get_test_reporter(), file, line, (tried == expected),
...
void assert_double_equal_(const char *file, int line, const char *expression, double tried, double expected) {
    (*get_test_reporter()->assert_true)(get_test_reporter(), file, line, doubles_are_equal(tried, expected),
...
void assert_string_equal_(const char *file, int line, const char *expression, const char *tried,
                          const char *expected) {
    (*get_test_reporter()->assert_true)(
        get_test_reporter(), file, line, strings_are_equal(tried, expected), "[%s] should be [%s] but was [%s]\n",
        expression, show_null_as_the_string_null(expected), show_null_as_the_string_null(tried));
}

void assert_string_not_equal_(const char *file, int line, const char *expression, const char *tried,
                              const char *expected) {
    (*get_test_reporter()->assert_true)(get_test_reporter(), file, line, !strings_are_equal(tried, expected),
                                        "[%s] should not be [%s] but was\n", expression,
                                        show_null_as_the_string_null(expected));
}

const char *show_null_as_the_string_null(const char *string) { return (string == NULL ? "NULL" : string); }
