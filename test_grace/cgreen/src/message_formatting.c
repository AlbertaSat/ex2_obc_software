...
static const char *next_percent_sign(const char *s) { return strchr(s, '%'); }
...
static bool actual_value_not_necessary_for(Constraint *constraint, const char *actual_string,
                                           const char *actual_value_string) {
    (void)constraint; // UNUSED!
    return strings_are_equal(actual_string, actual_value_string) || strings_are_equal(actual_string, "true") ||
...
char *validation_failure_message_for(Constraint *constraint, intptr_t actual) {
    const char *name_has_incorrect_size_message = "Wanted to compare contents with [%s],\n"
    ...
    size_t message_size = strlen(name_has_incorrect_size_message) + strlen(null_used_for_compare_message) +
                          strlen(null_used_for_actual_message) + 512; // just in case
    ...
    if (is_content_comparing(constraint)) {
        ...
        if ((long signed)constraint->size_of_expected_value <= 0) {
            snprintf(message + strlen(message), message_size - strlen(message) - 1,
                     name_has_incorrect_size_message, as_string, (long signed)constraint->size_of_expected_value);

            return message;
        }

        if ((void *)actual == NULL) {
            snprintf(message + strlen(message), message_size - strlen(message) - 1, null_used_for_compare_message,
                     as_string);
        ...
        if (constraint->expected_value.value.pointer_value == NULL) {
            snprintf(message + strlen(message), message_size - strlen(message) - 1, null_used_for_actual_message,
                     as_string);
...
char *failure_message_for(Constraint *constraint, const char *actual_string, intptr_t actual_value) {
    ...
    char *message;
    size_t message_size = strlen(constraint_as_string_format) + strlen(expected_value_string_format) +
                          strlen(actual_value_string_format) + strlen(at_offset) +
                          strlen(constraint->actual_value_message) + strlen(constraint->expected_value_message) +
                          strlen(constraint->expected_value_name) + strlen(constraint->name) +
                          strlen(actual_string) + 512; // just in case
    ...
    snprintf(message, message_size - 1, constraint_as_string_format, actual_value_as_string, constraint->name);
    ...
    snprintf(message + strlen(message), message_size - strlen(message) - 1, expected_value_string_format,
    ...
    if (values_are_strings_in(constraint)) {
        snprintf(message + strlen(message), message_size - strlen(message) - 1, actual_value_string_format,
                 (const char *)actual_value);
        if (!is_not_equal_to_string_constraint(constraint)) {
            strcat(message, "\n");
            snprintf(message + strlen(message), message_size - strlen(message) - 1,
                     constraint->expected_value_message, constraint->expected_value.value.string_value);
        }
    ...
    if (is_content_comparing(constraint)) {
        int difference_index = find_index_of_difference(constraint->expected_value.value.pointer_value,
                                                        (void *)actual_value, constraint->size_of_expected_value);
        if (difference_index != -1) {
            snprintf(message + strlen(message), message_size - strlen(message) - 1, at_offset, difference_index);
            snprintf(message + strlen(message), message_size - strlen(message) - 1, expected_content,
    ...
    snprintf(message + strlen(message), message_size - strlen(message) - 1, constraint->actual_value_message,
             actual_value);

    /* add the expected value */
    if (strstr(constraint->name, "not ") == NULL) {
        strcat(message, "\n");
        snprintf(message + strlen(message), message_size - strlen(message) - 1, constraint->expected_value_message,
