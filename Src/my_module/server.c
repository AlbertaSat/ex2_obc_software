...
/* NOTE: this function should really be in an interface file in CSP as it is
 * technically link layer */
...
void server_loop(void *parameters) {
  ...
  response_queue = xQueueCreate((unsigned portBASE_TYPE)RESPONSE_QUEUE_LENGTH,
  ...
  xTaskCreate((TaskFunction_t)response_server, "response server", 2048, NULL,
              RESPONSE_SERVER_PRIO, &response_server_task);
    ...
    while ((packet = csp_read(conn, 50)) != NULL) {
      switch (csp_conn_dport(conn)) {
        case TELECOMMAND_PORT:
          ...
          err =
              xQueueSendToBack(telecommand_queue, (void *)&incoming_telecommand,
