...
STX_return STX_getControl(uint8_t *pa, uint8_t *mode) {
    uint8_t rawValue = 0;
    if (read_reg(0x0, &rawValue) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
        *pa = rawValue >> 7;
        *mode = rawValue & 3;
...
STX_return STX_setControl(uint8_t new_pa, uint8_t new_mode) {
    ...
    if (write_reg(0x0, new_mode) != S_SUCCESS) {
        return S_BAD_WRITE;
    } else {
        return S_SUCCESS;
    }
...
STX_return STX_getEncoder(uint8_t *scrambler, uint8_t *filter, uint8_t *mod, uint8_t *rate) {
    uint8_t rawValue = 0;
    if (read_reg(0x01, &rawValue) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
        *rate = rawValue & 3;
        *mod = 1 & (rawValue >> 2);
...
STX_return STX_getPaPower(uint8_t *power) {
    ...
    if (read_reg(0x03, &rawValue) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
        switch (rawValue) {
        case 0:
...
STX_return STX_setPaPower(uint8_t new_paPower) {
    ...
    if (write_reg(0x03, rawValue) != S_SUCCESS) {
        return S_BAD_WRITE;
    } else {
        return S_SUCCESS;
    }
...
STX_return STX_getFrequency(float *freq) {
    uint8_t offset = 0;
    if (read_reg(0x04, &offset) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
...
STX_return STX_setFrequency(float new_frequency) {
    ...
    if (new_frequency >= 2200.0f && new_frequency <= 2300.0f) {
        ...
        if (write_reg(0x04, offset) != S_SUCCESS) {
            return S_BAD_WRITE;
        } else {
            return S_SUCCESS; // Successful Write
        }
    } else {
        return S_BAD_PARAM;
    }
...
STX_return STX_softResetFPGA(void) {
    if (write_reg(0x05, 0x0) != S_SUCCESS) {
        return S_BAD_WRITE;
    } else {
        return S_SUCCESS;
    }
...
STX_return STX_getFirmwareV(float *version) {
    uint8_t rawValue = 0;
    if (read_reg(0x11, &rawValue) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
...
STX_return STX_getStatus(uint8_t *pwrgd, uint8_t *txl) {
    uint8_t rawValue = 0;
    if (read_reg(0x12, &rawValue) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
        *pwrgd = (rawValue & 2) >> 1;
        *txl = rawValue & 1;
...
STX_return STX_getTR(int *transmit) {
    uint8_t rawValue = 0;
    if (read_reg(0x13, &rawValue) != S_SUCCESS) {
        return S_BAD_READ;
    } else {
        *transmit = rawValue & 1;
        return S_SUCCESS;
