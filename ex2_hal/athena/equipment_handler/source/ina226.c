...
int INA226_RegisterGet(i2cBASE_t *i2c, uint8_t addr, INA226_Register_TypeDef reg, uint16_t *val) {
    ...
    if (i2c_Send(i2c, addr, 1, &reg) == -1) {
        return -1;
    }

    if (i2c_Receive(i2c, addr, 2, &data) == -1) {
        return -1;
    }
...
int INA226_ReadShuntVoltage(i2cBASE_t *i2c, uint8_t addr, uint16_t *val) {
...
int INA226_ReadBusVoltage(i2cBASE_t *i2c, uint8_t addr, int *val) {
...
int INA226_ReadPower(i2cBASE_t *i2c, uint8_t addr, int *val) {
...
int INA226_ReadCurr(i2cBASE_t *i2c, uint8_t addr, int *val) {
