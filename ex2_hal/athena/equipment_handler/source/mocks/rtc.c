...
void mock_RTC_get_time(ex2_time_t *out_time) {
    memcpy(out_time, &utc_time, sizeof(ex2_time_t));
}

void mock_RTC_set_time(ex2_time_t new_time) {
    memcpy(&utc_time, &new_time, sizeof(ex2_time_t));
}

void mock_RTC_get_date(date_t *out_date) {
    memcpy(out_date, &utc_date, sizeof(date_t));
}

void mock_RTC_set_date(date_t new_date) {
    memcpy(&utc_date, &new_date, sizeof(date_t));
}
...
BaseType_t start_mock_rtc(TaskHandle_t *software_RTC_handle) {
    BaseType_t created = xTaskCreate((TaskFunction_t)mock_rtc_task, "Software_RTC", SOFTWARE_RTC_STACK_LEN, NULL, MOCK_RTC_TASK_PRIO, software_RTC_handle);
    return created;
}
