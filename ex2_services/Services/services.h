...
// needs to redifine
#define MAX_PKT_LEN                                               \
  210 /*ECSS_HEADER_SIZE + ECSS_DATA_HEADER_SIZE + MAX_PKT_DATA + \
         ECSS_CRC_SIZE*/

#define MAX_PKT_DATA 198
#define TC_MAX_PKT_SIZE 210
#define TC_MIN_PKT_SIZE \
  11  // 12  /*ECSS_HEADER_SIZE + ECSS_DATA_HEADER_SIZE + ECSS_CRC_SIZE*/
...
typedef enum {
  ...
  SATR_SCHS_TIM_SPEC_INVLD =
      26, /*Release time of TC is specified in a invalid representation*/
  SATR_SCHS_INTRL_LGC_ERR =
      27, /*The release time of telecommand is in the execution window of its
             interlocking telecommand.*/
  SATR_SCHS_DISABLED = 28,
  SATR_SCHS_NOT_IMLP = 29, /*Not implemented function of scheduling service*/
  /*FatFs*/
  SATRF_OK = 30, /* (0) Succeeded */
  SATRF_DISK_ERR =
      31, /* (1) A hard error occurred in the low level disk I/O layer */
  ...
  SATRF_DENIED =
      37, /* (7) Access denied due to prohibited access or directory full */
  ...
  SATRF_MKFS_ABORTED =
      44,             /* (14) The f_mkfs() aborted due to any parameter error */
  SATRF_TIMEOUT = 45, /* (15) Could not get a grant to access the volume within
                         defined period */
  SATRF_LOCKED = 46,  /* (16) The operation is rejected according to the file
                         sharing policy */
  SATRF_NOT_ENOUGH_CORE =
      47, /* (17) LFN working buffer could not be allocated */
...
#define TC_SU_MNLP_SERVICE \
  18 /*service number out of ECSS standard, mission specific for mnlp su*/
...
/*SCHEDULING SERVICE*/
#define SCHS_ENABLE_RELEASE                                                 \
  1 /*subservice 01, Telecommand to enable the release of telecommands from \
       schedule pool*/
#define SCHS_DISABLE_RELEASE                                                 \
  2 /*subservice 02, Telecommand to disable the release of telecommands from \
       schedule pool*/
#define SCHS_RESET_SCH \
  3 /*subservice 03, Telecommand to reset the schedule pool*/
#define SCHS_INSERT_TC_IN_SCH \
  4 /*subservice 04, Telecommand to insert a tc_tm_pkt in schedule pool*/
#define SCHS_DELETE_TC_FROM_SCH \
  5 /*subservice 05, Telecommand to delete a tc_tm_pkt from schedule pool*/
#define SCHS_DELETE_TC_FROM_SCH_OTP                                           \
  6 /*subservice 06, Telecommand to delete tc_tm_pkts from schedule pool over \
       a time period*/
#define SCHS_TIME_SHIFT_SEL_TC                                                \
  7 /*subservice 07, Telecommand to time shift (+/-) selected active schedule \
       packet*/
#define SCHS_TIME_SHIFT_SEL_TC_OTP                                            \
  8 /*subservice 08, Telecommand to time shift (+/-) selected active schedule \
       packets over a time period*/
#define SCHS_DETAILED_SCH_REPORT                                              \
  10 /*subservice 10, Telemerty response to (to TC no:16) report schedules in \
        detailed form*/
#define SCHS_SIMPLE_SCH_REPORT                                                \
  13 /*subservice 13, Telemerty response to (to TC no:17) report schedules in \
        summary form*/
#define SCHS_TIME_SHIFT_ALL_TCS                                           \
  15 /*subservice 15, Telecommand to time shift (+/-) all active schedule \
        packets*/
#define SCHS_REPORT_SCH_DETAILED \
  16 /*subservice 16, Telecommand to report schedules in detailed form*/
#define SCHS_REPORT_SCH_SUMMARY \
  17 /*subservice 17, Telecommand to report schedules in summary form*/
#define SCHS_LOAD_SCHEDULES \
  22 /*subservice 22, Telecommand to load schedules from perm storage*/
#define SCHS_SAVE_SCHEDULES \
  23 /*subservice 23, Telecommand to save schedules on perm storage*/
...
typedef enum {
  SU_NOSCRIPT =
      0, /*used when no script has ever been chosen to run, saved on SRAM*/
  SU_SCRIPT_1 = 1,
  SU_SCRIPT_2 = 2,
  SU_SCRIPT_3 = 3,
  SU_SCRIPT_4 = 4,
...
extern SAT_returnState event_crt_pkt_api(uint8_t *buf, uint8_t *f, uint16_t fi,
...
typedef struct {
  ...
  TC_TM_app_id app_id; /* TM: app id = 0 for time packets, = 0xff for idle packets. should be 11 bits only 8 are used though */ // REPLACE BY CSP ADDRESS
  ...
  uint8_t ser_type; /* 8 bit, service type */  // REPLACE WITH PORT NUMBER
   ...
   *verification service, when the packet wants ACK. the type is SAT_returnState
   *and it either stores R_OK or has the error code (failure reason). it is
   *initialized as R_ERROR and the service should be responsible to make it R_OK
   *or put the corresponding error.
   */
...
/*Lookup table that returns if a service with its subtype with TC or TM is
 * supported and valid*/
