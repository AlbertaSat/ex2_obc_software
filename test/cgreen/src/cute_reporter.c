...
static void cute_start_suite(TestReporter *reporter, const char *name, const int number_of_tests);
static void cute_start_test(TestReporter *reporter, const char *name);
...
static void cute_failed_to_complete(TestReporter *reporter, const char *file, int line, const char *message,
                                    va_list arguments);
...
static void cute_finish_suite(TestReporter *reporter, const char *filename, int line) {
    ...
    if (get_breadcrumb_depth((CgreenBreadcrumb *)reporter->breadcrumb) == 0) {
        memo->printer(": %d pass%s, %d failure%s, %d exception%s, %d ms.\n", reporter->passes,
                      reporter->passes == 1 ? "" : "es", reporter->failures, reporter->failures == 1 ? "" : "s",
                      reporter->exceptions, reporter->exceptions == 1 ? "" : "s", reporter->total_duration);
    } else
        memo->printer("\n");
}
