...
static void test_true(Constraint *constraint, const char *function, CgreenValue actual, const char *test_file,
...
static void set_contents(Constraint *constraint, const char *function, CgreenValue actual, const char *test_file,
                         int test_line, TestReporter *reporter);
...
Constraint *create_equal_to_value_constraint(intptr_t expected_value, const char *expected_value_name) {
    Constraint *constraint =
        create_constraint_expecting(make_cgreen_integer_value(expected_value), expected_value_name);
    constraint->type = CGREEN_VALUE_COMPARER_CONSTRAINT;
...
Constraint *create_equal_to_contents_constraint(void *pointer_to_compare, size_t size_to_compare,
                                                const char *compared_pointer_name) {
    Constraint *constraint =
        create_constraint_expecting(make_cgreen_pointer_value(pointer_to_compare), compared_pointer_name);
...
Constraint *create_equal_to_string_constraint(const char *expected_value, const char *expected_value_name) {
    Constraint *constraint =
        create_constraint_expecting(make_cgreen_string_value(expected_value), expected_value_name);
...
Constraint *create_does_not_contain_string_constraint(const char *expected_value,
...
Constraint *create_does_not_begin_with_string_constraint(const char *expected_value,
                                                         const char *expected_value_name) {
    Constraint *constraint =
        create_constraint_expecting(make_cgreen_string_value(expected_value), expected_value_name);
    constraint->type = CGREEN_STRING_COMPARER_CONSTRAINT;
...
Constraint *create_does_not_end_with_string_constraint(const char *expected_value,
                                                       const char *expected_value_name) {
    Constraint *constraint =
        create_constraint_expecting(make_cgreen_string_value(expected_value), expected_value_name);
    constraint->type = CGREEN_STRING_COMPARER_CONSTRAINT;
...
Constraint *create_equal_to_double_constraint(double expected_value, const char *expected_value_name) {
    Constraint *constraint =
        create_constraint_expecting(make_cgreen_double_value(expected_value), expected_value_name);
...
bool compare_want_contents(Constraint *constraint, CgreenValue actual) {
    ...
    return 0 == memcmp(constraint->expected_value.value.pointer_value, (void *)actual.value.integer_value,
...
static void set_contents(Constraint *constraint, const char *function, CgreenValue actual, const char *test_file,
    ...
    if (parameters_are_not_valid_for(constraint, actual.value.integer_value)) {
        ...
        (*reporter->assert_true)(reporter, test_file, test_line, false, message);

        free(message);
        return;
    }

    memmove((void *)actual.value.pointer_value, constraint->expected_value.value.pointer_value,
            constraint->size_of_expected_value);
}
...
static void execute_sideeffect(Constraint *constraint, const char *function, CgreenValue actual,
    ...
    if (constraint->side_effect_callback == NULL) {
        ...
        (*reporter->assert_true)(reporter, test_file, test_line, false, "no side effect function was set");
    }
    (constraint->side_effect_callback)(constraint->side_effect_data);
}

void test_want(Constraint *constraint, const char *function, CgreenValue actual, const char *test_file,
               int test_line, TestReporter *reporter) {
    char *message;
    ...
    snprintf(parameter_name_actual_string, sizeof(parameter_name_actual_string) - 1, "[%s] parameter in [%s]",
             constraint->parameter_name, function);
...
static void test_want_double(Constraint *constraint, const char *function, CgreenValue actual,
                             const char *test_file, int test_line, TestReporter *reporter) {
    (*reporter->assert_true)(reporter, test_file, test_line, (*constraint->compare)(constraint, actual),
                             "Wanted [%f], but got [%f] in function [%s] parameter [%s]",
                             constraint->expected_value.value.double_value, actual.value.double_value, function,
...
void destroy_double_constraint(Constraint *constraint) { destroy_empty_constraint(constraint); }
...
bool values_are_strings_in(const Constraint *constraint) {
    return is_string_comparing(constraint) && (constraint->expected_value.value.string_value != NULL);
}

bool no_expected_value_in(const Constraint *constraint) { return strlen(constraint->expected_value_message) == 0; }
...
bool is_not_content_setting(const Constraint *constraint) { return !is_content_setting(constraint); }
...
bool is_comparing(const Constraint *constraint) {
    return is_string_comparing(constraint) || is_content_comparing(constraint) ||
           is_double_comparing(constraint) || constraint->type == CGREEN_VALUE_COMPARER_CONSTRAINT;
}

bool is_not_comparing(const Constraint *constraint) { return !is_comparing(constraint); }
...
void significant_figures_for_assert_double_are(int figures) { significant_figures = figures; }

int get_significant_figures(void) { return significant_figures; }
