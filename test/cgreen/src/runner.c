...
static void run_every_test(TestSuite *suite, TestReporter *reporter) {
    ...
    reporter->duration =
        cgreen_time_duration_in_milliseconds(test_starting_milliseconds, cgreen_time_get_current_milliseconds());
...
static void run_named_test(TestSuite *suite, const char *name, TestReporter *reporter) {
    ...
    (*reporter->start_suite)(reporter, suite->name, count_tests(suite));
    for (i = 0; i < suite->size; i++) {
        if (suite->tests[i].type != test_function && has_test(suite->tests[i].Runnable.suite, name)) {
            (*suite->setup)();
...
static int per_test_timeout_defined() { return getenv(CGREEN_PER_TEST_TIMEOUT_ENVIRONMENT_VARIABLE) != NULL; }
