...
static RecordedExpectation *create_recorded_expectation(const char *function, const char *test_file, int test_line,
                                                        CgreenVector *constraints);
...
static void apply_any_read_only_parameter_constraints(RecordedExpectation *expectation, const char *parameter,
                                                      CgreenValue actual, TestReporter *test_reporter);
...
static void report_mock_parameter_name_not_found(TestReporter *test_reporter, RecordedExpectation *expectation,
...
static void apply_side_effect(TestReporter *test_reporter, const RecordedExpectation *expectation,
                              Constraint *constraint);

void cgreen_mocks_are(CgreenMockMode mock_mode) { cgreen_mocks_are_ = mock_mode; }
...
intptr_t mock_(TestReporter *test_reporter, const char *function, const char *mock_file, int mock_line,
    ...
    if (expectation == NULL) {
        handle_missing_expectation_for(function, mock_file, mock_line, parameter_names, actual_values,
                                       test_reporter);
    ...
    if (stored_result.type == CGREEN_DOUBLE) {
        ...
        /* TODO: for some future version we should ensure that the
           ...
           should be replaced by
               ...
               return mock_double(...);
        */
        test_reporter->assert_true(test_reporter, mock_file, mock_line, false,
...
static void apply_side_effect(TestReporter *test_reporter, const RecordedExpectation *expectation,
                              Constraint *constraint) {
    CgreenValue actual = {};
    constraint->execute(constraint, expectation->function, actual, expectation->test_file, expectation->test_line,
                        test_reporter);
}
static bool is_side_effect_constraint(const Constraint *constraint) {
...
static Constraint *create_appropriate_equal_constraint_for(const char *parameter_name, CgreenValue actual) {
    Constraint *constraint;
    if (actual.type == CGREEN_DOUBLE)
        constraint = create_equal_to_double_constraint(actual.value.double_value, parameter_name);
    else
        constraint = create_equal_to_value_constraint(actual.value.integer_value, parameter_name);
    return constraint;
}
...
void test_times_called(Constraint *constraint, const char *function, CgreenValue actual, const char *test_file,
    ...
    (*reporter->assert_true)(reporter, test_file, test_line, (*constraint->compare)(constraint, actual), message);
...
void expect_(TestReporter *test_reporter, const char *function, const char *test_file, int test_line, ...) {
    ...
    if (have_always_expectation_for(function)) {
        test_reporter->assert_true(
            test_reporter, test_file, test_line, false,
    ...
    if (have_never_call_expectation_for(function)) {
        ...
        test_reporter->assert_true(test_reporter, test_file, test_line, false,
...
static void report_violated_never_call(TestReporter *test_reporter, RecordedExpectation *expectation) {
    test_reporter->assert_true(test_reporter, expectation->test_file, expectation->test_line, false,
...
static bool successfully_mocked_call(const char *function_name) {
    ...
    for (i = 0; i < cgreen_vector_size(successfully_mocked_calls); i++) {
        const char *successfully_mocked_function_name =
            (const char *)cgreen_vector_get(successfully_mocked_calls, i);
...
static void report_unexpected_call(TestReporter *test_reporter, RecordedExpectation *expectation) {
    ...
    test_reporter->assert_true(test_reporter, expectation->test_file, expectation->test_line, false, message,
                               expectation->function);
}
...
static bool is_first_call_matching(RecordedExpectation *expectation) { return expectation->times_triggered == 0; }
...
static void trigger_unfulfilled_expectations(CgreenVector *expectation_queue, TestReporter *reporter) {
    for (int e = 0; e < cgreen_vector_size(expectation_queue); e++) {
        ...
        if (NULL == expectation) {
            fprintf(stderr,
                    "*** NULL unfulfilled cgreen expectation found -- maybe a previous incorrect removal?");
            continue;
        }
        ...
        if (is_never_call(expectation)) {
            if (is_first_call_matching(expectation)) {
                (*reporter->assert_true)(reporter, expectation->test_file, expectation->test_line, 1,
        ...
        for (int c = 0; c < cgreen_vector_size(expectation->constraints); c++) {
            Constraint *constraint = (Constraint *)cgreen_vector_get(expectation->constraints, c);
            if (constraint->type == CGREEN_CALL_COUNTER_CONSTRAINT) {
                constraint->execute(constraint, expectation->function,
                                    make_cgreen_integer_value(expectation->number_times_called),
                                    expectation->test_file, expectation->test_line, get_test_reporter());
                call_counter_present = true;
            }
        }

        if (!call_counter_present)
            (*reporter->assert_true)(reporter, expectation->test_file, expectation->test_line, 0,
...
static void report_mock_parameter_name_not_found(TestReporter *test_reporter, RecordedExpectation *expectation,
                                                 const char *constraint_parameter_name) {

    test_reporter->assert_true(test_reporter, expectation->test_file, expectation->test_line, false,
                               "Mocked function [%s] did not define a parameter named [%s]. Did you misspell it "
                               "in the expectation or forget it in the mock's argument list?",
                               expectation->function, constraint_parameter_name);
}
...
static void apply_any_read_only_parameter_constraints(RecordedExpectation *expectation, const char *parameter,
    ...
    for (i = 0; i < cgreen_vector_size(expectation->constraints); i++) {
        ...
        constraint->execute(constraint, expectation->function, actual, expectation->test_file,
                            expectation->test_line, test_reporter);
    }
}
