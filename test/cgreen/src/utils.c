...
/* For unittesting */
void panic_set_output_buffer(const char *buffer) { panic_message_buffer = (char *)buffer; }
...
void panic(const char *filename, int line, const char *fmt, ...) {
    ...
    va_start(args, fmt);
    sprintf(buffer, "%sCGREEN EXCEPTION%s: <%s:%d>", panic_use_colours ? MAGENTA : "",
            panic_use_colours ? RESET : "", filename, line);
