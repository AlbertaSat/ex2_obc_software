...
#define ENSURE_VA_NUM_ARGS(...)                                                                                   \
    ENSURE_VA_NUM_ARGS_IMPL_((__VA_ARGS__, _CALLED_WITH_TOO_MANY_ARGUMENTS, WithContextAndSpecificationName,      \
                              WithSpecificationName, DummyToFillVaArgs))
...
#define DescribeImplementation(subject)                                                                           \
    ...
    static void setup(void) {                                                                                     \
        if (BeforeEach_For_##subject != NULL)                                                                     \
            BeforeEach_For_##subject();                                                                           \
    }                                                                                                             \
    static void teardown(void) {                                                                                  \
        if (AfterEach_For_##subject != NULL)                                                                      \
            AfterEach_For_##subject();                                                                            \
    }                                                                                                             \
    typedef struct Dummy_##subject {                                                                              \
        int x;                                                                                                    \
    } Dummy_##subject##_t
