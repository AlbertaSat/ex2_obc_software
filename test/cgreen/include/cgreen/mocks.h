...
*/
#define expect(f, ...)                                                                                            \
    expect_(get_test_reporter(), STRINGIFY_TOKEN(f), FILENAME, __LINE__, (Constraint *)__VA_ARGS__ + 0,           \
            (Constraint *)0)
#define always_expect(f, ...)                                                                                     \
    always_expect_(get_test_reporter(), STRINGIFY_TOKEN(f), FILENAME, __LINE__, (Constraint *)__VA_ARGS__ + 0,    \
                   (Constraint *)0)
#define never_expect(f, ...)                                                                                      \
    never_expect_(get_test_reporter(), STRINGIFY_TOKEN(f), FILENAME, __LINE__, (Constraint *)__VA_ARGS__ + 0,     \
                  (Constraint *)0)

#ifdef _MSC_VER
// another workaround for fundamental variadic macro deficiencies in Visual C++ 2012
#define mock(...)                                                                                                 \
    PP_NARG(__VA_ARGS__)(get_test_reporter(), __func__, FILENAME, __LINE__, #__VA_ARGS__ "", __VA_ARGS__)
#else
#define mock(...)                                                                                                 \
    PP_NARG(__VA_ARGS__)(get_test_reporter(), __func__, FILENAME, __LINE__, #__VA_ARGS__ "", __VA_ARGS__ + 0)
#endif
