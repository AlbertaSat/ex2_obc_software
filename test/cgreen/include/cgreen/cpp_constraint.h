...
template <typename T> class CppConstraint : public Constraint {
  public:
...
Constraint *create_equal_to_string_constraint(const std::string *expected_value, const char *expected_value_name);
Constraint *create_not_equal_to_string_constraint(const std::string &expected_value,
...
Constraint *create_contains_string_constraint(const std::string *expected_value, const char *expected_value_name);
Constraint *create_does_not_contain_string_constraint(const std::string &expected_value,
                                                      const char *expected_value_name);
Constraint *create_does_not_contain_string_constraint(const std::string *expected_value,
                                                      const char *expected_value_name);
Constraint *create_begins_with_string_constraint(const std::string &expected_value,
                                                 const char *expected_value_name);
Constraint *create_begins_with_string_constraint(const std::string *expected_value,
                                                 const char *expected_value_name);

template <typename T> bool compare_want_value(CppConstraint<T> *constraint, T actual) {
    return constraint->expected_real_value == actual;
}

template <typename T> bool compare_do_not_want_value(CppConstraint<T> *constraint, T actual) {
    return !compare_want_value(constraint, actual);
}

template <typename T>
void test_want_value(CppConstraint<T> *constraint, const char *function, T actual, const char *test_file,
...
template <typename T>
CppConstraint<T> *create_equal_to_value_constraint(CgreenValue cgreen_value, T expected_value,
                                                   const char *expected_value_name) {
