...
static void telecommand_handler( void* parameters ) {
  ...
  for (;;) {
    if (xQueueReceive(telecommand_queue, (void*) &incoming_telecommand,
      ...
      printf("RX: %s, ID: %d\n", (char*) incoming_telecommand.data, incoming_telecommand.id);
    }
  }
}

int start_telecommand_handler() {
  telecommand_queue = xQueueCreate (
    (unsigned portBASE_TYPE) TELECOMMAND_QUEUE_LEN,
  ...
  xTaskCreate(
    (TaskFunction_t) telecommand_handler,
    "telecommand handler",
    2048,
    NULL,
    TELECOMMAND_HANDLER_PRIO,
    &telecommand_handler_task
  );
