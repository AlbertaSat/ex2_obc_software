...
void vSchedulerHandler(SemaphoreHandle_t scheduleSemaphore) {
    ...
    while (scheduler_stat.st_size >= sizeof(scheduled_commands_unix_t)) {
        ...
        if (cmds->unix_time > 0) {
            ...
            connect =
                csp_connect(CSP_PRIO_NORM, packet->id.dst, packet->id.dport, CSP_MAX_TIMEOUT, CSP_SO_HMACREQ);

            int send_embeddedPacket = csp_send(connect, packet, CSP_MAX_TIMEOUT);
            // TODO: find out if csp_send frees the packet, is so, the free packet above is not needed
