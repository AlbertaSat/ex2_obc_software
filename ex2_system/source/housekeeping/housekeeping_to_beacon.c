...
void update_beacon(All_systems_housekeeping *all_hk_data, beacon_packet_1_t *beacon_packet_one, beacon_packet_2_t *beacon_packet_two) {
    ...
    beacon_packet_two.obc_cmds_received = all_hk_data->Athena_hk.cmds_received;
    beacon_packet_two.pckts_unrecovered_by_FEC = all_hk_data->Athena_hk.pckts_uncovered_by_FEC;

    /*-------Logged Items-------*/
    // TODO: write the function for logged items in beacon_task
    beacon_packet_two.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet_two.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet_two.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet_two.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet_two.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
