...
static void state_daemon(void *pvParam) {
    for (;;) {
        ...
        uint8_t system_ctrl = 1; // A control word deciding what systems must be working
        ...
        case safe_state: // Battery heaters, EPS, OBC & UHF on
            system_ctrl |= (1 << batHeater_id) | (1 << EPS_id) | (1 << OBC_id) | (1 << UHF_id);
            // TODO: start a task that prepares for critical mode
            break;
        case operational_state: // All subsystems on
            system_ctrl |= (1 << batHeater_id) | (1 << EPS_id) | (1 << OBC_id) | (1 << UHF_id) | (1 << STX_id) |
                           (1 << Iris_id) | (1 << DFGM_id) | (1 << ADCS_id);
            break;
        default:
            // Shouldn't happen.
...
SAT_returnState start_state_daemon() {
    if (xTaskCreate((TaskFunction_t)state_daemon, "state_task", 512, NULL, STATE_TASK_PRIO, NULL) != pdPASS) {
